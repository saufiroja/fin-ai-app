...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const emailParam = urlParams.get('email') || localStorage.getItem('resetEmail') || '';
              ...
              <div className="p-4 rounded-lg bg-default-50 border border-default-200">
                ...
                <ul className="space-y-1 text-xs text-default-600">
                  <li className={`flex items-center gap-2 ${formData.password.length >= 8 ? 'text-success' : ''}`}>
                    <Icon 
                      icon={formData.password.length >= 8 ? "lucide:check" : "lucide:circle"} 
                      className="w-3 h-3" 
                    />
                  ...
                  <li className={`flex items-center gap-2 ${/[A-Z]/.test(formData.password) ? 'text-success' : ''}`}>
                    <Icon 
                      icon={/[A-Z]/.test(formData.password) ? "lucide:check" : "lucide:circle"} 
                      className="w-3 h-3" 
                    />
                  ...
                  <li className={`flex items-center gap-2 ${/\d/.test(formData.password) ? 'text-success' : ''}`}>
                    <Icon 
                      icon={/\d/.test(formData.password) ? "lucide:check" : "lucide:circle"} 
                      className="w-3 h-3" 
                    />
                    One number
                  </li>
